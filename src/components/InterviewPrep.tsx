...
const QuestionCard = memo(({ q, khLang }: { q: InterviewQuestion; khLang: boolean }) => {
  ...
  return (
    <div className={`rounded-xl border transition-colors ${expanded ? 'border-indigo-500/40 bg-indigo-500/5' : 'border-z-700/50 bg-z-800/30 hover:border-z-600'}`}>
      <button
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium leading-snug text-z-200">
            {khLang ? q.kh : q.en}
          </p>
          {!expanded && (
            <p className="text-xs text-z-500 mt-1 truncate">{q.tips[0]}</p>
          )}
        </div>
      </button>
      ...
      {expanded && (
        <div className="px-4 pb-4 pl-11 space-y-3">
          <div className="flex gap-2 text-xs text-z-500">
          ...
          <div className="space-y-2">
            ...
            {q.tips.map((tip, i) => (
              <div key={i} className="flex items-start gap-2">
                ...
                <p className="text-xs text-z-300 leading-relaxed">{tip}</p>
              </div>
            ))}
          </div>
        </div>
...
export const InterviewPrep = memo(({ onClose, onAskAI }: Props) => {
  ...
  return (
    ...
    >
      <div className="bg-z-900 border border-z-700 rounded-2xl w-full max-w-lg max-h-[85vh] flex flex-col shadow-2xl">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-z-700/50 shrink-0">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`px-2 py-1 rounded-lg text-[10px] font-bold border transition-colors ${khLang ? 'bg-indigo-600 text-white border-indigo-500' : 'bg-z-800 text-z-400 border-z-700 hover:text-z-200'}`}
            >
              {khLang ? 'EN' : 'KH'}
            </button>
            <button
              onClick={onClose}
              className="p-1.5 rounded-lg text-z-500 hover:text-z-200 hover:bg-z-800 transition-colors"
              aria-label="Close"
            >
              <X className="w-4 h-4" />
            </button>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
          ...
          {filtered.length === 0 && (
            <p className="text-sm text-z-500 text-center py-8">No questions in this category.</p>
          )}
        </div>

        <div className="px-5 py-3 border-t border-z-700/50 shrink-0 space-y-2">
          <button
            onClick={handleAskAI}
          ...
          <p className="text-xs text-z-500 text-center">
