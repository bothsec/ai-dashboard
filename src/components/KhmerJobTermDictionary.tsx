...
const TermCard = memo(({ term, onInsert }: { term: KhmerJobTerm; onInsert: (t: KhmerJobTerm) => void }) => (
  <button
    ...
    className="w-full text-left p-3 rounded-lg bg-zinc-800/50 hover:bg-zinc-800 border border-zinc-700/50 hover:border-amber-500/40 transition-all group"
  >
    <div className="flex items-start justify-between gap-2 mb-1">
      <div className="flex items-center gap-2">
        ...
        {term.kh && (
          <span className="text-xs text-zinc-500 font-hanuman">{term.kh}</span>
        )}
      </div>
      <ChevronRight className="w-3 h-3 text-zinc-600 group-hover:text-amber-500 transition-colors shrink-0 mt-0.5" />
    </div>
    <div className="text-[10px] text-blue-400 mb-1">{term.fullForm}</div>
    <div className="text-xs text-zinc-300 leading-relaxed">{term.meaning}</div>
    {term.example && (
      <div className="mt-1.5 text-[10px] text-zinc-500 italic border-l-2 border-zinc-700 pl-2 leading-relaxed">
        {term.example}
      </div>
    )}
  </button>
        ...
        <div className="bg-zinc-900 border border-zinc-700 rounded-xl w-full max-w-sm shadow-2xl">
          <div className="flex items-center justify-between p-4 border-b border-zinc-800">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-sm text-zinc-100">{selectedTerm.term}</span>
              {selectedTerm.kh && <span className="text-xs text-zinc-500 font-hanuman">{selectedTerm.kh}</span>}
            </div>
          ...
          <div className="p-4 space-y-3">
            <div>
              ...
              <div className="text-sm text-zinc-100 font-medium">{selectedTerm.fullForm}</div>
            </div>
            <div>
              <div className="text-[10px] text-blue-400 uppercase tracking-wider mb-1">Meaning</div>
              <div className="text-xs text-zinc-300 leading-relaxed">{selectedTerm.meaning}</div>
            </div>
            {selectedTerm.example && (
              <div>
                <div className="text-[10px] text-blue-400 uppercase tracking-wider mb-1">Example</div>
                <div className="text-xs text-zinc-400 italic leading-relaxed bg-zinc-800/50 rounded-lg p-2">{selectedTerm.example}</div>
              </div>
            )}
            <div>
    ...
    >
      <div className="bg-zinc-900 border border-zinc-700 rounded-xl w-full max-w-md max-h-[80vh] flex flex-col shadow-2xl">
        ...
        <div className="flex items-center justify-between p-4 border-b border-zinc-800">
          <div className="flex items-center gap-2">
            <HelpCircle className="w-4 h-4 text-amber-400" />
            <span className="font-semibold text-sm text-zinc-100">Khmer Job Term Dictionary</span>
          </div>
          <button onClick={onClose} className="text-zinc-500 hover:text-zinc-200 transition-colors">
            <X className="w-4 h-4" />
          </button>
        ...
        <div className="p-3 border-b border-zinc-800">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-8 pr-3 py-1.5 bg-zinc-800 border border-zinc-700 rounded-lg text-xs text-zinc-100 placeholder:text-zinc-500 focus:outline-none focus:border-amber-500"
              autoFocus
            />
          </div>
        </div>
        ...
        <div className="px-3 pt-2 pb-0 flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-zinc-600">{filtered.length} term{filtered.length !== 1 ? 's' : ''} found</span>
          <span className="text-[10px] text-zinc-700">·</span>
          <span className="text-[10px] text-zinc-600">Click to learn more, then ask AI</span>
        </div>
