...
export const SalaryPopover = memo(function SalaryPopover({ text, isDark, onClose }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className="p-2 space-y-1.5">
        {salaries.map((salary, i) => (
          <div key={i} className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs ${
            isDark ? 'bg-z-800' : 'bg-gray-50'
          }`}>
            <span className={`font-semibold ${isDark ? 'text-z-300' : 'text-gray-700'}`}>
              {salary.raw}
            </span>
            <ArrowRightLeft className={`w-3 h-3 shrink-0 ${isDark ? 'text-z-500' : 'text-gray-400'}`} />
            {salary.currency === 'USD' ? (
      ...
      {/* Insert hint */}
      <div className={`px-3 pb-2 text-[10px] ${isDark ? 'text-z-600' : 'text-gray-400'}`}>
