    ...
    >
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-zinc-200 leading-snug">{tip.title}</p>
      ...
      {expanded && (
        <div className="px-4 pb-4 pt-0">
          <div className="border-t border-zinc-700/30 pt-3 space-y-3">
            <div>
              <p className="text-xs font-semibold text-zinc-400 mb-1.5 uppercase tracking-wide">English</p>
              <div className="text-xs text-zinc-300 leading-relaxed whitespace-pre-wrap">{tip.body}</div>
            </div>
            <div>
              <p className="text-xs font-semibold text-amber-400/70 mb-1.5 uppercase tracking-wide">Khmer</p>
              <div className="text-xs text-zinc-400 leading-relaxed whitespace-pre-wrap font-hanuman">{tip.bodyKh}</div>
            </div>
            {tip.warning && (
...
export const WorkplaceTips = memo(function WorkplaceTips({ onClose }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* Panel */}
      <div className="relative w-full max-w-lg max-h-[88vh] overflow-hidden flex flex-col rounded-t-2xl sm:rounded-2xl border border-zinc-700 bg-zinc-900 shadow-2xl">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-700/50 shrink-0">
          <div>
            <h2 className="text-sm font-bold text-zinc-200">Khmer Workplace Tips</h2>
        ...
        <div className="flex-1 overflow-y-auto px-5 pb-5 space-y-2">
          {filtered.length === 0 ? (
            <p className="text-xs text-zinc-500 text-center py-8">No tips in this category yet.</p>
          ) : (
            filtered.map(tip => (
        ...
        {/* Footer */}
        <div className="shrink-0 px-5 py-3 border-t border-zinc-700/50 bg-zinc-900/80">
