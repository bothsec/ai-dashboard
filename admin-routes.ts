...
export async function initDatabase(): Promise<void> {
  ...
  // Load WASM binary via node_modules path (ESM-compatible with createRequire)
  ...
  if (count === 0) {
    ...
    _db.run('INSERT INTO users (username, password_hash, is_admin) VALUES (?, ?, 1)', [username, hash]);
  }
...
export function createSession(userId: number): string {
  ...
  db.run('INSERT INTO sessions (id, user_id, expires_at) VALUES (?, ?, ?)', [token, userId, expires]);
  saveDb();
  return token;
}
...
function cookieDomain(req: express.Request): string {
  ...
  const domain =
    typeof host === 'string' ? host.split(',')[0].split(':')[0] : undefined;
  if (
    domain &&
    /^(ai\.khmerjob\.tech|khmerjob\.tech|140\.238\.43\.61|localhost)$/.test(domain)
  ) {
...
export function registerAdminRoutes(app: express.Application): void {
  // POST /api/auth/login
    app.post('/api/auth/login', express.json(), (req, res) => {
      ...
      const row = rows[0]?.values[0];
      if (!row) {
        console.error('[auth] login fail: user not found', username);
        res.status(401).json({ error: { message: 'Invalid credentials' } });
        return;
      }
      if (!bcrypt.compareSync(password, row[1] as string)) {
        console.error('[auth] login fail: wrong password for', username);
        res.status(401).json({ error: { message: 'Invalid credentials' } });
        return;
      }
  ...
  app.patch('/api/admin/users/:id', express.json(), (req, res) => {
    ...
    if (is_active !== undefined) {
      getDb().run('UPDATE users SET is_active = ?, updated_at = datetime("now") WHERE id = ?', [
        is_active ? 1 : 0,
        id,
      ]);
    }
    if (password) {
      const hash = bcrypt.hashSync(password, 10);
      getDb().run('UPDATE users SET password_hash = ?, updated_at = datetime("now") WHERE id = ?', [
        hash,
        id,
      ]);
    }
  ...
  app.get('/api/admin/users/:id/features', (req, res) => {
    ...
    const rows = getDb().exec('SELECT feature_name, enabled FROM user_features WHERE user_id = ?', [
      userId,
    ]);
